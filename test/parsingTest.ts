...
import { LookupItem } from '../src/models';
import { Parser } from '../src/parsing';
import * as assert from 'assert';

let parseLineTests: { [id: string]: LookupItem } = {};

function addParseLineTest(input: string, name: string, quantity: number) {
	parseLineTests[input] = new LookupItem(name, quantity);
}
...
describe('Parser', () => {
	describe('parseLine', () => {
		for (let test in parseLineTests) {
			it(`should parse "${test}"`, () => {
				let result = Parser.parseLine(test);
	...
	describe('parse', () => {
		it('should successfully parse', () => {
			let results = Parser.parse(multiLineFittingString);
